...
from lib.models.prithvi_combined import PrithviCombined
...
def main():
	...
	parser.add_argument("--emb_dim", type=int, default=128,
	                   help="Embedding dimension for conv3 temporal path")
	parser.add_argument("--n_layers", type=int, default=3,
	                   help="Number of conv3 temporal blocks")
	parser.add_argument("--dropout", type=float, default=0.1,
	                   help="Dropout rate for conv3 path")
	...
	model = PrithviCombined(
		config["pretrained_cfg"], weights_path, True,
		n_classes=4, model_size=args.model_size,
		emb_dim=args.emb_dim, n_layers=args.n_layers, dropout=args.dropout,
		c_per_t=args.c_per_t, feed_timeloc=args.feed_timeloc,
	)
	...
	for epoch in range(config["training"]["n_iteration"]):
		...
		for j,batch_data in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
			...
			if j%10==0:
				...
				to_print = f"Epoch: {epoch}, iteration: {j}, loss: {loss.item()}, alpha: {alpha_val:.4f} \n "
				print(to_print)
		...
		print("="*100)
		to_print = f"Epoch: {epoch}, val_loss: {epoch_loss_val}, alpha: {model.alpha.item():.4f} \n "
		for idx in range(4):
	...
	if args.logging:
		...
		wandb.run.summary[f"final_alpha"] = model.alpha.item()
