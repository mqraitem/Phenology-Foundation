...
from lib.models.prithvi_blowup_improved_noshuffle import PrithviSegBlowupImprovedNoShuffle
...
def main():
	...
	parser.add_argument("--c_per_t", type=int, default=8,
	                   help="Channels per timestep after PixelShuffle (e.g. 4 -> 48ch at full res)")
	parser.add_argument("--hidden_dim", type=int, default=768,
	                   help="Hidden dimension in temporal conv layers at full resolution")
	parser.add_argument("--n_temporal_layers", type=int, default=8,
	                   help="Number of residual Conv2d layers in the temporal fusion head")
	parser.add_argument("--separate_heads", type=str2bool, default=True,
	                   help="Use separate prediction heads per phenology stage")
	parser.add_argument("--temporal_attention", type=str2bool, default=True,
	                   help="Use SE-style temporal attention before temporal conv fusion")
	...
	model=PrithviSegBlowupImprovedNoShuffle(
		config["pretrained_cfg"], weights_path, True,
		n_classes=4, model_size=args.model_size,
		c_per_t=args.c_per_t, hidden_dim=args.hidden_dim,
		n_temporal_layers=args.n_temporal_layers,
		separate_heads=args.separate_heads,
		temporal_attention=args.temporal_attention,
