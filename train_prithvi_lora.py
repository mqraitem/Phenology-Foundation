...
def main():
	...
	parser.add_argument("--alpha", type=int, default=16,
	                   help="Alpha scaling parameter for LoRA")
	...
	# Freeze entire encoder (including LoRA) - train only decoder head first
	model.backbone.model.model.encoder.eval()
	for param in model.backbone.model.model.encoder.parameters():
		param.requires_grad = False

	print("Initial state - only decoder head trainable:")
	...
	for epoch in range(config["training"]["n_iteration"]):
		...
		if epoch == 1:
			...
			for name, param in model.backbone.model.model.encoder.named_parameters():
				if "lora_" in name:
					param.requires_grad = True

			print("UnFreezing LoRA parameters only")
			print("="*100)
			print_trainable_parameters(model)
